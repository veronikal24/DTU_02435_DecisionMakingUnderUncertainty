...
# Electrolyzer switch constraints
def electrolyzer_switch_on(model, t):
    if t > 0:
        return model.E_SW[t] >= (model.E_ON[t] - model.E_ON[t-1])
    return Constraint.Skip
model.electrolyzer_switch_on_constr = Constraint(range(T), rule=electrolyzer_switch_on)
